...
от своего имени (организация, контрагент, внешняя система — «заказчик услуг», \
«подрядчик», «сервис нарядов») — теми же словами, что в тексте. Затем для \
...
или внешняя система (сервис нарядов, подрядчик, адресат). Один пул — один \
процесс.
...
(упаковщик, фасовщик, наладчик, технолог, мастер участка, начальник смены) \
пулом быть не может: это всегда дорожка организации, \
которой она подчиняется. Пулов в схеме не больше, чем организаций и внешних \
систем в описании.
- Действие внешнего участника принадлежит его пулу: «подрядчик подтверждает \
наряд», «сервис выдаёт номер заявки», «заказчик услуг подписывает акт» — шаги \
...
Агрегатор заявок», пул называется «Агрегатор заявок», а не «система»: родовое \
имя («система», «сервис», \
...
- Имя пула — то же слово, что в описании: названная система «Агрегатор заявок» \
остаётся «Агрегатор заявок», а не превращается в «Цех фасовки». Схема, где \
участник переименован, теряет \
связь с описанием, и проверить её взаимодействие нечем.
- Каждый участник, названный в описании (организация, контрагент, внешняя \
система — в том числе аббревиатура вроде SAP, CRM, 1С), обязан присутствовать \
...
«Агрегатор принимает заявку и отправляет её подрядчику» — это два шага, а не \
один «Приём и передача заявки»; последнее действие фразы («и отправляет её \
подрядчику») тоже остаётся на схеме. Склеенные или выброшенные действия — это \
...
обработки: поток в шаг-эскалацию, а не обратно в ту же задачу. Входящего \
потока у него не бывает: хозяина будит `attached_to`, поэтому дуга «шаг → это \
событие» — ошибка маршрута, а не связь.
...
- Программа — пул только тогда, когда в описании она сама совершает действие
(«модуль учёта закрывает наряд», «датчик включает останов»). Если ею лишь
пользуются («наряд заводят в модуле», «акт вносят в журнал», «список выгружают
из программы») — это инструмент организации, а не участник: действие
принадлежит тому, кто действует, и записывается дорожкой его пула.
...
Ни один утверждённый пул не остаётся без своего шага: действие, которое по
описанию делает участник, живёт в его пуле, а не в пуле того, кто этого действия
ждёт, — ждать — не значит делать. «Заказчик ждёт подтверждения от контрагента» —
это шаг заказчика; подтверждение, которое даёт контрагент, — шаг пула
контрагента.
"""
...
#
# Имена в примере — из домена, которого нет в eval-сценариях («Сервисная
# служба», «Слесарь-ремонтник»), иначе заплатка отдаёт модели ответ оракула:
# «Перевозчик» и «Водитель» в этом месте были требованием сцены
# warehouse_delivery, и `expected_participants` мерил копирование. Провенанс
...
"condition": "Да", "kind": "sequence"}],
 "remove_flows": ["F3"],
 "participants": [{"name": "Сервисная служба", "external": true,
                   "steps": ["A8"]}],
 "lanes": [{"id": "L9", "name": "Слесарь-ремонтник",
            "participant": "Сервисная служба"}]}"""
...
- `event_definition`, `timer` и `attached_to` — поля узла-события. Превращать
  шаг маршрута в граничное событие правкой `fixes` нельзя: у шага остаются его
  входящие и исходящие потоки, и план после такой правки противоречит себе.
  Таймер добавляют отдельным узлом: `add_elements` с `kind=boundaryEvent`,
  `attached_to` на нужный шаг и `event_definition`=`timer`.
...
class BPMNGenerator:
    ...
    def _clarify_ownership(self, text: str,
        ...
        question = _OWNERSHIP_TEMPLATE.format(
            ...
            # Связь «роль ↔ пустой пул той же организации» модель видит не
            # всегда: в прогоне #50 `warehouse_delivery` r0/r2 шаги перевозчика
            # остались за его же ролью-пулом, пустой пул организации удалился
            # починкой, и один дефект уронил `expected_participants` вместе с
            # `roles_as_lanes`. Имена пустых пулов уже есть первым списком в
            # этом же вопросе — здесь они названы явно как возможные хозяева.
            vacant_hosts=("" if not vacant else
                          "\nХозяин роли может быть среди пустых пулов выше: "
                          + ", ".join(f"«{v['pool']}»" for v in vacant)
                          + ". Если роль работает на такой пул, верни его имя в "
                            "`inside` — шаги роли станут его дорожкой, и "
                            "участник из описания останется на схеме.\n"),
    ...
    for pool in _vacant_pools(raw):
        if pool in named_vacant:
            ...
            gaps.append(f"пул «{pool}» без единого шага — в нём только старт "
                        ...
                        f"его нельзя: верни его действия в его пул — правь "
                        f"`participant=\"{pool}\"` у тех шагов, которые по "
                        "описанию делает он, а не записывай их чужим "
                        "участником"
