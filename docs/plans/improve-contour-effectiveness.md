...
## Ход тридцать третьего прохода: экономическая проверка собственной правки (#61)

**#61 (`--mode live --repeat 2`, транспорт живой, отказов сборки 0).** Правка
прошлого хода «не удалять пул, названный в описании» оказалась слишком широкой, и
прогон это показал по двум направлениям сразу:

| метрика | #59 | #61 |
|---|---|---|
| `pass@1/expected_participants` | 0.75 | **0.812** |
| `pass@1/has_branching` | 0.625 | **0.75** |
| `pass@1/pool_has_steps` | 1.0 | **0.562** |
| `quality/scheme_checks_share` | 0.876 | 0.821 |
| `improve/defects_repaired` | 0.238 | **0.411** |
| `improve/error_share` | 0.30 | **0.10** |
| `improve/op_acceptance` | 0.652 | 0.481 |

Участников контур стал терять меньше (`expected_participants` +0.06,
`has_branching` +0.125), и пакет улучшения чинит охотнее (`defects_repaired`
0.238 → 0.411 при `error_share` 0.30 → 0.10: в выборку перестали попадать схемы,
где пула уже нет, а правка должна ссылаться на его id). Но цена — семь пустых
пулов из шестнадцати схем вместо нуля: `pool_has_steps` упал до 0.562, и сводка
гейта стала хуже на 0.055. Причина ровно та, о которой предупреждал текст
`_claim_steps_by_name`: удалять нечего, если наполнять нечем.

**Правило сужено до исполнимой части (коммит `734c53d`).** Пустой пул остаётся
на схеме, только если в чужом пуле лежит шаг, названный в его честь, — то есть
когда перенос физически существует и его можно потребовать и узким вопросом, и
пакетом (`participant="имя"` из инвентаря). Названного в описании участника с
такой парой контур больше не теряет; пул-на-всякий-случай, которому наполнять не
чем, по-прежнему удаляется, иначе `pool_has_steps` становится вечным.

Тесты: выживающий пул собирается с кандидатом («Собрать заказ кладовщиком» в
чужом пуле), отдельный контракт требует удаления названного пула без кандидата и
цитирует цены #61. 1197 тестов, replay-гейт 0 (сводка гейта 0.929,
`pass@1/pool_has_steps` 1.0, `pass@1/expected_participants` 0.846).

---

## Ход тридцать четвёртого прохода: правка откатана по измерению

**#62 (`--mode live --repeat 2`, 16 сцен + пакеты, транспорт живой).** Прогон
дописал к истории то, чего я не предвидел: правка прошлых двух ходов не просто
переложила дефект между инвариантами, она ухудшила приёмку.

| метрика | #59 | #61 | #62 |
|---|---|---|---|
| `pass@1/expected_participants` | 0.75 | 0.812 | **0.25** |
| `pass@1/has_branching` | 0.625 | 0.75 | **0.875** |
| `pass@1/pool_has_steps` | 1.0 | 0.562 | 0.625 |
| `quality/scheme_checks_share` | **0.876** | 0.821 | 0.811 |
| `improve/scheme_checks_share` | 0.883 | 0.845 | 0.861 |
| `improve/op_acceptance` | 0.652 | 0.481 | 0.621 |

Откатаны оба изменения (`b065c01`, вместе с их тестами и переписанными
контрактами):

* оставление пустого пула названного участника — даже в суженном виде «только
  если его шаги лежат в чужом пуле»; второй прогон показал, что сужение не
  спасает, потому что пустые пулы приходят не только из `_drop_vacant_pools`;
* признак сказуемого для имён в начале фразы — он действительно находит
  «Клиента» и «Перевозчика», но расширяет список нарушений плана, а на расширенный
  список модель отвечает переписанным составом и теряет уже утверждённых
  участников: 12 провалов `expected_participants` вместо 3.

Что осталось от той же гипотезы — только то, что измерение поддержало:
сохранение ветвления (`has_branching` 0.625 → 0.75 → 0.875, монотонно три
прогона подряд) и подсказки об операндах. Вывод записан в `AGENTS.md`: потерю
участника надо чинить наполнением пула (узкий вопрос о принадлежности, пакет с
операндами), а не ценой пустых прямоугольников.

Контур потерял две правки и приобрёл одну, встанющую отдельно от гипотез:
`remove_lane` принят алиасом к `delete`, `delete` снимает пустую дорожку и
отвергает наполненную со списком переносимых id (`f9e195d`). Класс из двух
отказов «неизвестная операция» в #61 закрыт на уровне словарей, а не промпта.

Тесты: 1194, replay-гейт 0.

---

## Ход тридцать пятого прохода: после отката — лучший live и починенная метрика повтора

**#63 (`--mode live --repeat 2`, 16 сцен + пакеты).** Состояние после `b065c01`
(откат оставления пустых пулов и признака сказуемого) плюс алиасы, подсказки об
операндах и `remove_lane`:

| метрика | #61 | #62 | #63 |
|---|---|---|---|
| `quality/scheme_checks_share` | 0.821 | 0.811 | **0.879** |
| `pass@1/pool_has_steps` | 0.562 | 0.625 | **1.0** |
| `pass@1/has_branching` | 0.75 | 0.875 | 0.875 |
| `improve/scheme_checks_share` | 0.845 | 0.861 | **0.903** |
| `improve/op_acceptance` | 0.481 | 0.621 | 0.567 |
| `improve/error_share` | 0.10 | 0.20 | 0.15 |
| `pass@1/expected_participants` | 0.812 | 0.25 | 0.375 |

Откат подтверждён измерением: сводка гейта вернулась к уровню #59, пустые пулы —
к нулю, монотонность `has_branching` (0.625 → 0.75 → 0.875 → 0.875) уцелела,
потому что её держит другая правка. Не вернулся к #59 уровень единственный ряд,
ради которого всё и затевалось: `expected_participants` 0.375 при 10 проваленных
строках из 16. Это уже не механика починки — модель не заводит пул для
действующего лица, даже когда узкий вопрос ей задан (строки `gaps` это называют),
и живой разброс 0.25–0.81 на одном и том же коде показывает, что дальше рядом
идти нельзя: следующий lever обязан быть проверен двумя прогонами, иначе он
выглядит улучшением.

**Вторая находка хода — сама метрика `improve/retry_gain`.** Она отдавала наружу
абсолютное число закрытых повтором отказов и агрегировалась средним: 0.0 в #59 и
#61 читалось как «корректирующий повтор бессилен». После перевода в долю
(`e7ab3df`) тот же #63 даёт 0.357 — повтор закрывает треть названных отказов.
Знаменатель в метрику не входил, и вывод о всём механизме делался по числу без
единицы измерения. Отказы второго раунда в знаменатель не попадают намеренно:
они описывают сам повтор, а не то, что ему предшествовало.

Замер live-прогонов теперь сопоставим между собой: `eval/baselines/live.json`
пишется отдельным прогоном с `--write-baseline --baseline-path`, и несовпадение
режимов по-прежнему сверку пропускает.

### Ход тридцать шестой прохода: сверка документации со слепком

Числа в README оказались рассказом про прогон #63, тогда как сверяются они с
записанным слепком #65 (`eval/baselines/live.json`), и живой разброс сделал
расхождение заметным: 0.903 против 0.875 по пакету, 0.567 против 0.519 по
доле принятых операций. Одна строка была просто не на своём месте: число
«1.062 запаха на схему» стояло в ряду `quality/business_checks_share`, где живёт
доля, а не счёт. Таблица переведена на значения слепка, к ней добавлен ряд
`business/smells` с явно направленной вниз меркой.

**Разбор отказов #65 — где остаток, а где контур.** Из 85 отказов 15 приходятся
на `connect` с причиной «источник или цель не найдены» и 4 на
`add_event_definition` с «элемент не найден». Инвентарь схемы (узлы и потоки,
раздел `flows`) в промпт улучшения попадает, отказ формулируется как просьба
взять id из него, то есть это не провал словаря и не провал аплайера, а выбор
модели: она адресует правку несуществующему узлу. Классы, закрытые в этом блоке
работ (алиасы словаря, операнд слияния участников, снятие пустой дорожки), от
этого ряда не зависят — они держатся на отказе «неизвестная операция», а не на
адресе. Второй по величине ряд — законные продуктовые запреты (`delete` стартового
события, `add_boundary_event` без ветки обработки, `add_lane` с id без `new_`),
и они называют формулировку, которой модель держится за легальную правку.

**Класс «отверг уже сделанное» схлопнулся.** Тот же разбор по всем живым отчётам
даёт по пиковому прогону 33 повторных отказа из 121 и 2 из 85 в записанном.
Метрика `improve/redundant_refusals_share` это видит, но в слепки она не попала
(заряжена позже записи) и в replay считается по одному кейсу — поэтому в
документации она рядом с числом кейсов, а не вместо него.
