...
class TestAgreement:
    ...
    def test_divergences_on_real_schemes_are_the_documented_ones(self, agreement):
        """Остаётся одно расхождение: `rework_loop` — место вопроса (ноги
        развилки у оракула против дуг, отпускающих цикл, у скоринга).
        `lane_overload` вычеркнуто по измерению: оракул взял тот же
        двухпороговый признак (60% при трёх дорожках, 75% при двух), и 18 схем
        корпуса, где скоринг ругался в одиночку, сошлись. Новое имя здесь значит,
        что линейки разъехались, а не что так задумано."""
        assert coverage.unexplained(agreement) == []
        assert set(coverage.disagreements(agreement)) == {"rework_loop"}
...
class TestNotationAgreement:
    ...
    `participant_interacts` стоит в гейте `scenario_pass`), и 2 схемы, где
...
class TestDenialMatchesTheApplier:
    ...
    отказ честный. Ratchet нужен, чтобы он таким и остался: молчание про
