...
447 тестов: разбор ответов LLM, генерация структуры и починка плана модели
(пустые пулы, слияние «ролей-пулов» в дорожки, вставка шлюзов схождения,
определения событий и таймеры), аплайер операций (включая откат шагов,
оставшихся вне маршрута, и откат пакета, замкнувшего цикл без выхода),
RAG-поиск по эталонам, скоринг по 16 правилам, HTTP-контур
...
Качество ИИ-контура измеряет отдельно харнесс `eval/`:
...
python -m eval.run --write-baseline              # зафиксировать baseline
```

Он прогоняет записанные ответы модели через настоящий конвейер и считает
`pass@1` по структурным инвариантам (пул без шагов, роль вместо дорожки,
развилка без схождения, событие без определения, таймер без ветки обработки),
число правок починки на схему, средний балл и его разброс, долю применённых
операций улучшения и дельту балла. Инварианты — независимый оракул, а не
тот же скоринг, иначе правка скоринга двигала бы и линейку, и измеряемое
значение. `--baseline` сравнивает прогон с `eval/baselines/current.json` и
возвращает ненулевой код при просадке метрик больше порога.
