...
- У каждого элемента — шага, шлюза и события — заполни `documentation` одним \
предложением из текста описания: что происходит, кто делает, при каком условии. \
У стартового события — что его запускает, у завершающего — каким состоянием \
заканчивается процесс, у граничного или ожидающего — чего ждут и что бывает по \
истечении. Документация, дословно повторяющая имя элемента, не считается: \
имя шага для чтения процесса не заменить описанием — схему правят, согласуют \
и ищут по описаниям элементов, а не по заголовкам.
...
{"id": "L_p", "name": "Начальник смены", "participant": "Цех фасовки"}],
 "elements": [
   {"id": "S1", "kind": "startEvent", "name": "Наряд на починку", "participant": "Цех фасовки", "lane": "L_p", "documentation": "Начальник смены зарегистрировал наряд — отсчёт процесса с этого момента"},
   ...
   {"id": "G1", "kind": "exclusiveGateway", "name": "Деталь в ремфонде?", "participant": "Цех фасовки", "lane": "L_t", "documentation": "Проверка наличия детали: есть — меняем сразу, нет — узел встаёт в план заказа"},
   ...
   {"id": "T1", "kind": "boundaryEvent", "name": "Прошло 6 часов", "participant": "Цех фасовки", "lane": "L_t", "attached_to": "A2", "event_definition": "timer", "timer": "PT6H", "documentation": "Замена длится дольше шести часов — сигнал доложить мастеру, работы не сворачиваются"},
   ...
   {"id": "E1", "kind": "endEvent", "name": "Линия в работе", "participant": "Цех фасовки", "lane": "L_p", "documentation": "Узел заменён, линия снова в работе, наряд закрыт"},
