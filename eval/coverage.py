...
# Расхождения, объявленные в замысле, а не найденные промахи. `lane_overload`:
# у скоринга порог 60% при трёх дорожках и калитка «дорожек хотя бы три», у
# оракула — 75% без калитки, и в docstrings это записано. `rework_loop`: слои
...
# всегда «скоринг строже». `gateway_split_join` так и не попал в этот список:
