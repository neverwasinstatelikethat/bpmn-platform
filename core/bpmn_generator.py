...
class BPMNGenerator:
    ...
    def _clarify_ownership(self, text: str,
        ...
        # Спрашиваем про трёх кандидатов с большим числом своих шагов: такой пул
        # остаётся на схеме пулом и раздувает их число, а `roles_as_lanes` мерит
        # именно его. Прежний порядок «у кого меньше шагов» живой прогон #40
        # потратил на пустых контрагентов («перевозчик», «получатель») — хозяина у них нет, и
        # модель на них не ответила ни разу; роли-обёртки с шагами остались без
        # вопроса. Длинный список модель закрывает реже (см.
        # MAX_ROLE_QUESTIONS), поэтому ответ про любого другого кандидата плана
        # принимается так же — `candidates` ниже строится по полному списку.
